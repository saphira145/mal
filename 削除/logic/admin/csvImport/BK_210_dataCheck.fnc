<?php
	...
	====================================================== */
		...
		function check_preferentialTaxType ( $value, $key ) {
			...
			// 有無存在チェック
			if( !preferentialTaxTypeExist( $value ) && count( $err ) == 0 ) {
				$err["iniExist"] = 1;
			}
	...
	====================================================== */
		...
		/*----------------------------------------------------
		    優遇税制区分チェック
		----------------------------------------------------*/
		function preferentialTaxTypeExist ( $value ) {

			global $ini;

			for( $i=0; $i<count( $ini["preferentialTaxType"] ); $i++ ) {
				if( $ini["preferentialTaxType"][$i]["value"] == $value ) {
					return true;
				}
			}
